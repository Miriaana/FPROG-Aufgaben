...
// Custon Test Cases
/*The difference in the number of tokens for the two players >1
Check for correct tokens
Check for wrong tokens
Board-dimension is wrong
Board-dimension is correct*/
TEST_CASE("The difference in the number of tokens for the two players >1"){
    ...
    Line concatLine = concatenate3(line(board, 0), line(board, 1), line(board, 2));

    auto ValidDifferenceOfPlayerTokens = [](const Line line) -> bool{
        return ( 
            1 >= (abs(accumulateAll(line, 
                [](int sum, char token) -> int {
                    return sum + (token == 'X') - (token == 'O');
                }
            )))
        );
    };
    ...
    int expected = false;
    CHECK_EQ(expected, ValidDifferenceOfPlayerTokens(concatLine));
}
...
TEST_CASE("Check for correct tokens"){
    ...
    auto TokensAreValid = [](const Board board){
        return (
            all_of_collection(board, 
                [](const auto innerVector){
                    return all_of_collection(innerVector, 
                        [](char token){
    ...
    int expected = true;
    CHECK_EQ(expected, TokensAreValid(board));
}
...
TEST_CASE("Board-dimension is wrong"){
    ...
    auto AreBoardDimensionsValid = [](const Board board){
        return all_of_collection(board, 
                [](const auto InnerVector){
                    return InnerVector.size() == 3;
                }
            ) && board.size() == 3;
    };

    int expected = false;
    CHECK_EQ(expected, AreBoardDimensionsValid(board));
}
